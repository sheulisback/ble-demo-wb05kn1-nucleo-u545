/**
  ******************************************************************************
  * @file    stm32wb05n_events_cb.c
  * @author  System Research & Applications Team - Agrate/Catania Lab.
  * @version V1.3.0
  * @date    06-Feb-2024
  * @brief   Event callbacks for external uC - STM32WB05N in network coprocessor 
  *          mode
  ******************************************************************************
  * @attention
  *
                                      ...
                                      uint8_t Reason))
{
   ...
   */
  STM32WB_PRINTF("hci_disconnection_complete_event\r\n");
}
/**
                                 ...
                                 uint8_t*  PDU_Data)
{
  /* NOTE : This function Should not be modified, when needed,
            the callback could be implemented in the user file
   */
  STM32WB_PRINTF("hci_rx_acl_data_event\r\n");
  return 0;
}
/**
                                    ...
                                    uint8_t *ISO_Data_Load)
{
  /* NOTE : This function Should not be modified, when needed,
            the callback could be implemented in the user file
   */
  STM32WB_PRINTF("hci_le_rx_iso_data_event\r\n");
  return 0;
}
/**
 * @}
                                    ...
                                    uint8_t Reason))
{
  /* NOTE : This function Should not be modified, when needed,
            the callback could be implemented in the user file
   */
  STM32WB_PRINTF("aci_gap_pairing_complete_event\r\n");
}
 ...
 *        the application has to respond with the @ref aci_gap_passkey_resp
 *        command.
 * @param Connection_Handle Connection handle for which the passkey has been
 *        requested.
 * @param Display_Input This parameter informs the application if the passkey
 *        needs to be displayed or requested for input on the local device.
 *        Values:
...
/* Event len: 2 + 1 */
/**
 * @brief This event may be generated when there is a request to start a pairing
 *        process.  Application shall respond with aci_gap_pairing_resp command
 *        to accept or reject the incoming pairing procedure notified through
 *        this event.   If the pairing is going to start with a non-bonded
 *        device, the Bonded parameter is set to 0. Instead, if the pairing
 *        process is going to start with an already bonded device, the event is
 *        raised with Bonded parameter set to 1. This may happen either if the
 *        peer has lost the bond or if it is a malicious device.   If
 *        aci_gap_set_security_requirements command was given with
 *        Pairing_Response set to 1 (pairing confirmation only for bonded
 *        devices), the event is raised only if pairing is going to be initiated
 *        with a bonded device.
...
/**
 * @brief This event is generated when the central responds to the connection
...
/**
 * @brief This event is generated when the central does not respond to the
 ...
 *        is received from the peripheral. The upper layer which receives this
 *        event has to respond by sending a @ref
 ...
 *        Values:
 *        - 0x01 ... 0x05
 * @param Local_CID Array of up to 5 two-octet values that represent the local
 *        channel endpoints identifying the EATT channels to be reconfigured.
 * @retval None
 */
WEAK_FUNCTION(void aci_l2cap_cos_reconfiguration_event(uint16_t Connection_Handle,
                                         uint8_t Event_Type,
                                         uint16_t Result,
                                         uint8_t Identifier,
                                         uint16_t Peer_MTU,
...
/**
 * @brief This event is generated when the central rejects a L2CAP request.
 * @param Connection_Handle Handle identifying the connection.
 * @param Identifier This is the identifier which associates the request to the
 *        response.
 * @param Reason Reason
...
/* Event len: 2 + 1 + 1 + 2 + 2 + 2 + 2 + 1 */
/**
 * @brief Event generated when a request is received from the peer to create one
 *        L2CAP Credit Based Flow Control channel or one or more (up to 5) L2CAP
 *        Enhanced Credit Based Flow Control channels.
 * @param Connection_Handle Connection handle that identifies the connection.
 *        Values:
 *        - 0x0000 ... 0x0EFF
...
WEAK_FUNCTION(void aci_l2cap_cos_connection_req_event(uint16_t Connection_Handle,
...
/* aci_l2cap_cos_connection_resp_event */
/* Event len: 2 + 1 + 2 + 2 + 2 + 2 + 1 + rp0->CID_Count * (sizeof(uint16_t)) */
/**
 * @brief Event generated when a response is received from the peer to create
 *        one L2CAP Credit Based Flow Control channel or one or more (up to 5)
 ...
 *        layer entity of the peer device as soon as the L2CAP channel is
 *        established.
 * @param Result Outcome of the connetion request.
 *        Values:
 ...
 *        Values:
 *        - 0x01 ... 0x05
 * @param CID Array of up to 5 two-octet values that represent the local channel
 *        endpoints.
 * @retval None
 */

WEAK_FUNCTION(void aci_l2cap_cos_connection_resp_event(uint16_t Connection_Handle,
                                              uint8_t Channel_Type,
                                              uint16_t Peer_MTU,
                                              uint16_t Peer_MPS,
                                              uint16_t Initial_Credits,
                                           ...
                                           uint8_t Attr_Data[]))
{
  /* NOTE : This function Should not be modified, when needed,
            the callback could be implemented in the user file
   */
  STM32WB_PRINTF("aci_gatt_srv_attribute_modified_event\r\n");
}
/**
 ...
 *        or from the peer), which can happen only on an unenhaced ATT bearer.
 *        See aci_gatt_clt_exchange_config().
 * @param Connection_Handle Connection handle related to the response
 * @param MTU ATT_MTU value agreed between server and client. This is the
 ...
 *        or was successful. The Central_Clock_Accuracy parameter is only valid
 ...
 *        peer device
 * @param Connection_Interval Connection interval used on this connection. Time
 *        = N * 1.25 msec
 *        Values:
 *        - 0x0006 (7.50 ms)  ... 0x0C80 (4000.00 ms) 
                                      ...
                                      uint8_t Central_Clock_Accuracy))
{
  /* NOTE : This function Should not be modified, when needed,
            the callback could be implemented in the user file
   */
  STM32WB_PRINTF("hci_le_connection_complete_event\r\n");
}
/**
 ...
 *        Controller process to update the connection has completed. On a
 *        peripheral, if no connection parameters are updated, then this event
 *        shall not be issued. On a central, this event shall be issued if the
 ...
 *        completion of the process of the Controller obtaining the used
...
/**
 * @brief The LE Long Term Key Request event indicates that the central device
 *        is attempting to encrypt or re-encrypt the link and is requesting the
                                               ...
                                               uint8_t Central_Clock_Accuracy))
{
  /* NOTE : This function Should not be modified, when needed,
            the callback could be implemented in the user file
   */
  STM32WB_PRINTF("hci_le_enhanced_connection_complete_event\r\n");
}
/**
 * @brief The HCI_LE_Directed_Advertising_Report event indicates that directed
 *        advertisements have been received where the advertiser is using a
 *        resolvable private address for the TargetA field of the advertising
 *        PDU which the Controller is unable to resolve and the
 *        Scanning_Filter_Policy is equal to 0x02 or 0x03. Direct_Address_Type
 *        and Direct_Address specify the address the directed advertisements are
 *        being directed to. Address_Type and Address specify the address of the
 *        advertiser sending the directed advertisements. The Controller may
 *        queue these advertising reports and send information from multiple
 *        advertisers in one HCI_LE_Directed_Advertising_Report event. This
 *        event shall only be generated if scanning was enabled using the
 *        HCI_LE_Set_Scan_Enable command. It only reports advertising events
 *        that used legacy advertising PDUs.
 * @param Num_Reports Number of responses in this event.
 *        Values:
 *        - 0x01
...
/**
 * @brief The HCI_LE_BIGInfo_Advertising_Report event indicates that the
 *        Controller has received an Advertising PDU that contained a BIGInfo
 *        field. If the Controller also generates an
 *        HCI_LE_Periodic_Advertising_Report event, the
 *        HCI_LE_BIGInfo_Advertising_Report event shall immediately follow that
 *        event. An HCI_LE_BIGInfo_Advertising_Report event shall be generated
 *        even if the Controller is already synchronized to the BIG. The
 *        Sync_Handle parameter shall identify the periodic advertising train
 *        containing the BIGInfo field and shall be the same as the
 *        corresponding field in the HCI_LE_Periodic_Advertising_Report event if
 *        one is generated. The Num_BIS, NSE, ISO_Interval, BN, PTO, IRC,
 *        Max_PDU, SDU_Interval, Max_SDU, PHY, and Framing parameters correspond
 *        to the associated fields in the BIGInfo field of the Advertising PDU.
 *        If the BIGInfo field indicates that the corresponding BIG is
 *        encrypted, the Encryption parameter shall be set to 0x01. Otherwise,
...
/* Event len: 1 + 2 + 1 + 1 + 6 + 1 + 2 + 1 + 1 + 1 + 1 + 1 */
/**
 * @brief The HCI_LE_Periodic_Advertising_Sync_Established event indicates that
 *        the Controller has received the first periodic advertising packet from
 *        an advertiser after the HCI_LE_Periodic_Advertising_Create_Sync
 *        command has been sent to the Controller. The Sync_Handle parameter
 *        identifies the periodic advertising train in subsequent commands and
 *        events and shall be assigned by the Controller. The Advertising_SID
 *        parameter is set to the value of the Advertising SID subfield in the
 *        ADI field of the advertising PDU referring to the periodic advertising
 *        train. The Advertiser_Address_Type and Advertiser_Address parameters
 *        specify the address of the periodic advertiser. The Advertiser_PHY
 *        parameter specifies the PHY used for the periodic advertising. The
 *        Periodic_Advertising_Interval parameter specifies the interval between
 *        the periodic advertising events. The Advertiser_Clock_Accuracy
 *        parameter specifies the accuracy of the periodic advertiser's clock.
 *        If the periodic advertising has subevents or response slots, then the
 *        Num_- Subevents, Subevent_Interval, Response_Slot_Delay, and
 *        Response_Slot_- Spacing specify the parameters for these subevents,
 *        otherwise these values shall be set to 0x00.
 * @param Status For standard error codes see Bluetooth specification, Vol. 2,
 *        part D. For proprietary error code refer to Error codes section.
 * @param Sync_Handle Sync handle that identifies the synchronization
 *        information about the periodic advertising train.
 ...
 *        Values:
 *        - 0x00 ... 0x80
 * @param Subevent_Interval Subevent interval. Time = N x 1.25 ms.
 *        Values:
 *        - 0x00 (NaN) : No subevents
 *        - 0x06 (7.50 ms)  ... 0xFF (318.75 ms) 
 * @param Response_Slot_Delay Response slot delay. Range: 0x01 to 0xFE. Time = N
 *        x 1.25 ms
 *        Values:
 *        - 0x00 (NaN) : No response slots
 *        - 0x01 (1.25 ms)  ... 0xFE (317.50 ms) 
 * @param Response_Slot_Spacing Response slot spacing Range: 0x02 to 0xFF Time =
 *        N x 0.125 ms
 *        Values:
...
WEAK_FUNCTION(void hci_le_periodic_advertising_sync_established_v2_event(uint8_t Status,
                                                              uint16_t Sync_Handle,
                                                              uint8_t Advertising_SID,
                                                              uint8_t Advertiser_Address_Type,
                                                              uint8_t Advertiser_Address[6],
...
/* Event len: 2 + 1 + 1 + 1 + 2 + 1 + 1 + 1 + rp0->Data_Length * (sizeof(uint8_t)) */
/**
 * @brief The HCI_LE_Periodic_Advertising_Report event indicates that the
 *        Controller has received a periodic advertisement or has failed to
 *        receive an AUX_SYNC_SUBEVENT_IND PDU. The Sync_Handle parameter
 *        identifies the periodic advertising train that the report relates to.
 *        The RSSI parameter contains the RSSI value, excluding any Constant
 *        Tone Extension. If the Controller supports the Connectionless CTE
 *        Receiver feature, RSSI shall not be set to 0x7F. When multiple
 *        advertising packets are used to complete a periodic advertising report
 *        (e.g., a packet containing an AUX_SYNC_IND PDU combined with one
 *        containing an AUX_CHAIN PDU), the RSSI event parameter shall be set
 *        based on the last packet received and the TX_Power event parameter
 *        shall be set based on the AUX_SYNC_IND PDU. However, the second or
 *        subsequent events for the same periodic advertisement may instead have
 *        a TX_Power value of 0x7F. The Controller may split the data from a
 *        single periodic advertisement (whether one PDU or several) into
 *        several reports. If so, each report except the last shall have a
 *        Data_Status of "incomplete, more data to come", while the last shall
 *        have the value "complete". No further reports shall be sent for a
 *        given periodic advertisement after one with a Data_Status other than
 *        "incomplete, more data to come". A Data_Status of "incomplete, data
 *        truncated" indicates that the Controller attempted to receive an
 *        AUX_CHAIN_IND PDU but was not successful or received it but was unable
 *        to store the data. The CTE_Type parameter indicates the type of
 *        Constant Tone Extension in the periodic advertising packets. The
 *        Periodic_Event_Counter parameter indicates the periodic advertising
 *        event counter (paEventCounter) of the event that the periodic
 *        advertising packet was received in. The Subevent parameter indicates
 *        the Periodic Advertising with Responses subevent that the periodic
 *        advertising packet was received in. If the Periodic Advertising does
 *        not have subevents, then Subevent shall be set to 0xFF. If the
 *        Controller receives an AUX_CHAIN_IND PDU with no AdvData, it should
 *        send the report (or the last report if it has split the data)
 ...
 *        - 0xFF: No Constant Tone Extension
 * @param Periodic_Event_Counter The value of paEventCounter (see [Vol 6] Part
 *        B, Section 4.4.2.1) for the reported periodic advertising packet.
...
WEAK_FUNCTION(void hci_le_periodic_advertising_report_v2_event(uint16_t Sync_Handle,
                                                    int8_t TX_Power,
                                                    int8_t RSSI,
                                                    uint8_t CTE_Type,
                                                    uint16_t Periodic_Event_Counter,
...
/* Event len: 1 + 2 + 2 + 2 + 1 + 1 + 6 + 1 + 2 + 1 + 1 + 1 + 1 + 1 */
/**
 * @brief The HCI_LE_Periodic_Advertising_Sync_Transfer_Received event is used
 *        by the Controller to report that it has received periodic advertising
 *        synchronization information from the device referred to by the
 ...
 *        assigned by the Controller. The remaining parameters provide
 *        information about the periodic advertising (see Section 7.7.65.14). If
 *        there are no subevents or response slots, then the Controller shall
 *        set the Num_Subevents parameter to zero and the Host shall ignore the
 *        Subevent_Interval, Response_Slot_Delay, and Response_Slot_- Spacing
 *        parameters. If Status is non-zero, all parameter values are valid
 *        except Sync_Handle, which the Host shall ignore. Note: If the
 *        Controller is already synchronized to the periodic advertising train
...
WEAK_FUNCTION(void hci_le_periodic_advertising_sync_transfer_received_v2_event(uint8_t Status,
                                                                      uint16_t Connection_Handle,
                                                                      uint16_t Service_data,
                                                                      uint16_t Sync_Handle,
                                                                      uint8_t Advertising_SID,
...
/* Event len: 1 + 1 + 1 */
/**
 * @brief The HCI_LE_Periodic_Advertising_Subevent_Data_Request event is used to
 *        allow the Controller to indicate that it is ready to transmit one or
 *        more subevents and is requesting the advertising data for these
 *        subevents. The Subevent_Data_Count parameter shall be less than or
 *        equal to the number of subevents. The Subevent_Start parameter is the
 *        first subevent being requested and the Subevent_Data_Count parameter
 *        determines the subsequent subevents being requested. The subevent
 *        numbers wrap from one less than the number of subevents to zero. This
 *        event should be sent from the Controller when it has no data for
 *        upcoming subevents. The Controller should request data for as many
 *        subevents as it has memory to accept to minimize the number of events
...
/* Event len: 1 + 1 + 1 + 1 + struct_length */
/**
 * @brief The HCI_LE_Periodic_Advertising_Response_Report event indicates that
 *        one or more Bluetooth devices have responded to a periodic advertising
 *        subevent during a PAwR train. The Controller may queue these
 *        advertising reports and send information from multiple devices in one
 *        HCI_LE_Periodic_Advertising_- Response_Report event. The Controller
 *        may fail to transmit the synchronization packet required to enable the
 *        response packets to be sent. If this happens, the Controller can
 *        report this to the Host using the Tx_Status parameter. The Controller
 *        may split the data from a single response into several reports. If so,
 *        each report except the last shall have a Data_Status of "incomplete,
 *        more data to come", while the last shall have the value "complete". No
 *        further reports shall be sent for a given periodic advertisement after
 *        one with a Data_Status other than "incomplete, more data to come".
 * @param Advertising_Handle Used to identify a periodic advertising train.
 * @param Subevent The subevent number.
 * @param Tx_Status If AUX_SYNC_SUBEVENT_IND was transmitted or not.
...
/* Event len: 1 + 2 + 1 + 1 + 6 + 6 + 6 + 2 + 2 + 2 + 1 + 1 + 2 */
/**
 * @brief The HCI_LE_Enhanced_Connection_Complete event indicates to both of the
 *        Hosts forming the connection that a new connection has been created.
 *        Upon the creation of the connection a Connection_Handle shall be
 *        assigned by the Controller, and passed to the Host in this event. If
 *        the connection creation fails, this event shall be provided to the
 *        Host that had issued the HCI_LE_Create_- Connection or
 *        HCI_LE_Extended_Create_Connection command. If this event is unmasked
 *        and the HCI_LE_Connection_Complete event is unmasked, only the
 *        HCI_LE_Enhanced_Connection_Complete event is sent when a new
 *        connection has been created. This event indicates to the Host that
 *        issued an HCI_LE_Create_Connection or
 ...
 *        successful. The Peer_Address, Peer_Resolvable_Private_Address, and
 *        Local_- Resolvable_Private_Address shall always reflect the most
 *        recent packet sent and received on air. The Central_Clock_Accuracy
 *        parameter is only valid for a Peripheral. On a Central, this parameter
 *        shall be set to 0x00. If the connection is established from periodic
 *        advertising with responses and Role is 0x00, then the
 *        Advertising_Handle parameter shall be set according to the periodic
 *        advertising train the connection was established from. If the
 *        connection is established from periodic advertising with responses and
 *        Role is 0x01, then the Sync_Handle parameter shall be set according to
 *        the periodic advertising train the connection was established from. In
 *        all other circumstances, Advertising_Handle and Sync_Handle shall be
 *        set to No Advertising_Handle and No Sync_Handle and shall be ignored
 *        by the Host.
 * @param Status For standard error codes see Bluetooth specification, Vol. 2,
 *        part D. For proprietary error code refer to Error codes section.
 * @param Connection_Handle Connection handle to be used to identify the
...
WEAK_FUNCTION(void hci_le_enhanced_connection_complete_v2_event(uint8_t Status,
                                                  uint16_t Connection_Handle,
                                                  uint8_t Role,
                                                  uint8_t Peer_Address_Type,
                                                  uint8_t Peer_Address[6],
