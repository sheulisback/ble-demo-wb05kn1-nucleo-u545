/**
  ******************************************************************************
  * @file    stm32wb05n_gatt_aci.c
  * @author  System Research & Applications Team - Agrate/Catania Lab.
  * @version V1.3.0
  * @date    06-Feb-2024
  * @brief   Source file for external uC - STM32WB05N in network coprocessor mode (gatt_aci)
                                          ...
                                          uint16_t *Char_Desc_Handle)
{
  ...
  /* var_len_data input */
  {
    ...
    STM32WB_memcpy((void *) &cp0->Char_Desc_Uuid, (const void *) Char_Desc_Uuid, size);
    index_input += size;
    {
    ...
    STM32WB_memcpy((void *) &cp1->Char_Desc_Value, (const void *) Char_Desc_Value, Char_Desc_Value_Length*sizeof(uint8_t));
                                          ...
                                          uint8_t Attribute_Val[])
{
  ...
  /* var_len_data input */
  {
    STM32WB_memcpy((void *) &cp0->Attribute_Val, (const void *) Attribute_Val, Attribute_Val_Length*sizeof(uint8_t));
                                                     ...
                                                     UUID_t *UUID)
{
  ...
  /* var_len_data input */
  {
    ...
    STM32WB_memcpy((void *) &cp0->UUID, (const void *) UUID, size);
    index_input += size;
  }
  STM32WB_memset(&rq, 0, sizeof(rq));
  rq.ext_aci = TRUE;
                                                 ...
                                                 uint16_t Handle[])
{
  ...
  /* var_len_data input */
  {
    STM32WB_memcpy((void *) &cp0->Handle, (const void *) Handle, Number_of_Handles*sizeof(uint16_t));
    index_input += Number_of_Handles*sizeof(uint16_t);
  }
  STM32WB_memset(&rq, 0, sizeof(rq));
  rq.ext_aci = TRUE;
  rq.ogf = 0x3f;
                                              ...
                                              uint8_t Value[])
{
  ...
  *Value_Length = btoh(resp.Value_Length, 2);
  STM32WB_memcpy((void *) Value, (const void *) resp.Value, *Value_Length*sizeof(uint8_t));
  return BLE_STATUS_SUCCESS;
}
                                     ...
                                     Gatt_Srv_Notify_Attr_t Gatt_Srv_Notify_Attr[])
{
  ...
  /* var_len_data input */
  {
    for (int i = 0; i < Num_Of_Attr; i++) {
      ...
      index_input += 2;
      STM32WB_memcpy(Gatt_Srv_Notify_Attr_tmp->Val, Gatt_Srv_Notify_Attr[i].Val, Gatt_Srv_Notify_Attr[i].Val_Length);
                                                            ...
                                                            uint8_t Sec_Level)
{
  struct hci_request rq;
  uint8_t cmd_buffer[532];
  aci_gatt_clt_add_subscription_security_level_nwk_cp0 *cp0 = (aci_gatt_clt_add_subscription_security_level_nwk_cp0*)(cmd_buffer);
  tBleStatus status = 0;
  uint8_t index_input = 0;
  cp0->Conn_Handle = htob(Conn_Handle, 2);
  index_input += 2;
  cp0->Char_Value_Handle = htob(Char_Value_Handle, 2);
  index_input += 2;
  cp0->Sec_Level = htob(Sec_Level, 1);
  index_input += 1;
  STM32WB_memset(&rq, 0, sizeof(rq));
  rq.ext_aci = TRUE;
  rq.ogf = 0x3f;
