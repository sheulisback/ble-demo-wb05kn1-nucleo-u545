/**
  ******************************************************************************
  * @file    stm32wb05n_hci_le.h
  * @author  System Research & Applications Team - Agrate/Catania Lab.
  * @version V1.3.0
  * @date    06-Feb-2024
  * @brief   Header file for external uC - STM32WB05N in network coprocessor mode (hci_le)
...
#ifndef _STM32WB05N_HCI_LE_H_
#define _STM32WB05N_HCI_LE_H_
...
tBleStatus hci_le_set_extended_advertising_parameters_v2(uint8_t Advertising_Handle,
                                                         ...
                                                         int8_t *Selected_Tx_Power);
/**
 * @brief The HCI_LE_Set_Periodic_Advertising_Subevent_Data command is used by
 *        the Host to set the data for one or more subevents of PAwR in reply to
 *        an HCI_LE_Periodic_Advertising_Subevent_Data_Request event. The data
 *        for a subevent shall be transmitted only once. The Advertising_Handle
 *        parameter identifies the advertising set whose periodic advertising
 *        subevent data is being set. If the corresponding advertising set does
 *        not already exist, then the Controller shall return the error code
 *        Unknown Advertising Identifier (0x42). The Num_Subevents parameter is
 *        the number of subevent data contained in the parameter arrays. The
 *        Subevent arrayed parameter identifies the subevent of the PAwR that is
 *        being set. If the Host provides a subevent value that is outside of
 *        the range of subevents requested using the
 *        HCI_LE_Periodic_Advertising_Subevent_Data_Request event, then the
 *        Controller shall return the error code Command Disallowed (0x0C). The
 *        Response_Slot_Start and Response_Slot_Count arrayed parameters
 *        identify the starting response slot and the number of response slots
 *        that are expected to be used in this subevent. The
 *        Subevent_Data_Length arrayed parameter determines the length of the
 *        Subevent_Data that is significant. The Subevent_Data arrayed parameter
 *        contains the advertising data to be transmitted in the subevent of the
 *        advertising set. If the combined data length is greater than the
 *        maximum that the Controller can transmit within the current subevent
 *        interval, then all data shall be discarded and the Controller shall
 *        return the error code Packet Too Long (0x45). If advertising on the LE
 *        Coded PHY, then the S=8 coding shall be assumed unless the current
 *        advertising parameters require the use of S=2 for an advertising
 *        physical channel, in which case the S=2 coding shall be assumed for
 *        that advertising physical channel. If the Subevent_Data cannot be
 *        transmitted because, for example, the subevent where this data would
 *        have been sent has already passed or is too early, then the Controller
 *        shall return the error code Too Late (0x46) or Too Early (0x47) and
 *        discard the data.
 * @param Advertising_Handle It is used to identify an advertising set
 *        Values:
 *        - 0x00 ... 0xEF: Used to identify an advertising set
 ...
 *        Values:
 *        - 0x01 ... 0x0F
 * @param Subevent_Data_Parameters See @ref Subevent_Data_Parameters_t
 * @retval Value indicating success or error code.
 */
...
/**
 * @brief The HCI_LE_Set_Periodic_Advertising_Response_Data command is used by
 ...
 *        Values:
 *        - 0x00 ... 0xFB
 * @param Response_Data Response data formatted as defined in [Vol 3] Part C,
 *        Section 11.
 * @retval Value indicating success or error code.
 */
